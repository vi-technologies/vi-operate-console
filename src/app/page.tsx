...
export default function HomePage() {
  ...
  const viProducts = [
    {
      title: 'Vi Activate',
      description: 'Uses the power of AI to identify, predict, and convert the highest value prospects at the lowest CPA',
    ...
    {
      title: 'Vi Engage',
      description: 'Increases the lifetime value of a member by optimizing enrollment, engagement, and retention',
    ...
    {
      title: 'Vi Operate',
      description: 'Leverages Generative AI to maximize operational efficiencies and financial returns',
      ...
      <div className={`relative z-20 flex flex-col items-center justify-center min-h-screen px-4 py-12 transition-opacity duration-1000 ${isLoaded ? 'opacity-100' : 'opacity-0'}`}>
        <div className="text-center mb-12 pt-16">
          ...
          <p className="text-xl text-gray-300 max-w-2xl mx-auto animate-fade-in">
            Helping people live healthy and active lives through AI-powered solutions
          </p>
        </div>
            ...
            <Link href={product.href} key={index}>
              <Card className="h-full bg-black/40 border border-gray-800 backdrop-blur-sm hover:bg-black/60 transition-all duration-300 group overflow-hidden">
                <CardContent className="p-6">
                  <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${product.color} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300`}>
                    {product.icon}
                  </div>
        ...
        <div className="animate-fade-in flex gap-4">
          <Link href="/about">
            <Button variant="outline" className="px-6 py-6 text-lg border-primary text-primary hover:bg-primary/10">
              Learn More
            </Button>
          </Link>
          <Link href="/console">
